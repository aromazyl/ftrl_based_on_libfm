...
double fm_model::predict(const sparse_row<FM_FLOAT>& x, DVector<double> &sum, DVector<double> &sum_sqr) const {
	double result = 0;
	if (k0) {	
		result += w0;
	}
...
double fm_model::predict(const sparse_row<FM_FLOAT>& x) {
