...
#include "src/fm_learn_ftrl.h"
#include "src/fm_learn_ftrl_element.h"
...
int main(int argc, char **argv) { 
  ...
  try {
    ...
    const std::string param_method		= cmdline.registerParameter("method", "learning method (SGD, SGDA, ALS, MCMC, FTRL); default=MCMC");
    ...
    } else if (!cmdline.getValue(param_method).compare("ftrl")){
      fml = new fm_learn_ftrl_element();
      ((fm_learn_ftrl*)fml)->num_iter = cmdline.getValue(param_num_iter, 100);
    } else {
